...
def train_v4(ep, model, optimizer, lr_scheduler, train_loader, device, config):
    lr_scheduler.step()
    model.train()
    ...
    for data, target, make_target, type_target in train_loader:
        ...
        print(f'Epoch {ep:03d} [{i}/{len(train_loader)}]: '
              f'Loss: {loss_meter / i:.4f} '
              f'Type_Acc: {make_acc_meter / i:.4f} '
              f'({elapsed:.2f}s)', end='\r')
...
def main(args):
    ...
    if config['version'] == "make":
        for ep in range(1, config['epochs'] + 1):
            ...
            if best_acc < valres['val_make_acc']:
                best_acc = valres['val_make_acc']
                torch.save(model.state_dict(), exp_dir + '/best.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version', default=1, type=int, choices=[1, 2, 3, "type", "make"],
