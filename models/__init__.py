...
from models.network_v4 import TypeClassifier, MakeClassifier, NetworkV4
...
def construct_model(config, num_classes, num_makes, num_types):
    ...
    elif config['version'] == 'type':
        model = TypeClassifier(base, num_classes, num_makes, num_types)
    elif config['version'] == 'make':
        model = MakeClassifier(base, num_classes, num_makes, num_types)
